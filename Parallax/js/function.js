$(window).scroll(function() {
    ...
    $(".fore-bird").css({
        ...
        // offset artinya memperoses objeck yang di scroll 
    if(wScroll > $('.clothes-pics').offset().top - ($(window).height() / 1.2)) {
        $('.clothes-pics figure').each(function(i) {
            // setTimeout artinya adalah Don't do anything
            setTimeout(function() {
    ...
    if(wScroll > $('.large-window').offset().top - $(window).height()) {
        $('.large-window').css({'background-postion':'center '+ (wScroll - $('.large-window').offset().top) +'px'});
